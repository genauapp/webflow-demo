...
export function mountBookmarkAutocomplete(inputEl, getWords, onSelect) {
  ...
  dropdown.style =
    'position: absolute; z-index: 10; background: #fff; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); margin-top: 4px; min-width: 220px; display: none;'
  ...
  inputEl.addEventListener('input', () => {
    ...
    if (suggestions.length && value.length > 1) {
      suggestions.forEach((suggestion) => {
        ...
        item.style =
          'padding: 8px 16px; cursor: pointer; font-size: 15px; display: flex; justify-content: space-between; align-items: center;'
        const tag = document.createElement('span')
        tag.textContent = suggestion.tag
        tag.style =
          'margin-left: 8px; background: rgb(0, 0, 0); color: rgb(255, 255, 255); font-size: 13px; border-radius: 6px; padding: 2px 8px;'
