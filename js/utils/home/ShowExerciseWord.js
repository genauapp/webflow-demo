...
export default function showExerciseWord() {
  ...
  const learnedWithExerciseWords = LocalStorageManager.load(
    LEARNED_WITH_EXERCISE_WORDS_KEY,
  ...
  const inProgressWords = LocalStorageManager.load(
    IN_PROGRESS_WORDS_KEY,
    DEFAULT_VALUE.IN_PROGRESS_WORDS
  )
  const totalWordsExercise = LocalStorageManager.load(
    TOTAL_WORD_EXERCISE_KEY,
    DEFAULT_VALUE.TOTAL_WORD_EXERCISE
  )
  // updated indexes
  document.getElementById(`remainingWordsCountExercise-${wordType}`).innerText =
    learnedWithExerciseWords[currentLevel][category][wordType].length
  ...
  const { word, english, level } = currentWord
